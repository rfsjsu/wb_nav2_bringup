#!/usr/bin/env python3
"""
Mission 5: Evaluation - Repeated pallet docking with variable pallet orientations relative
...
Here the left/right position of the pallet is varied relative to the
dock pose set in the docking server.

Usage:
    python3 mission_r05.py [--rounds N] [--pallets P1 P2 ...] [--destinations D1 D2 ...]

Examples:
    python3 mission_r05.py --rounds 1
    python3 mission_r05.py --rounds 10
    python3 mission_r05.py --rounds 5 --pallets P1 P2 --destinations D2
"""
import sys
import os
import time
...
def pallet_shift(shift):
    """
    Shift the pallet along the y-axis.
    Input is centimeters.
    """
    for pallet in PALLET_ORIGINS:
        PALLET_ORIGINS[pallet]['y'] = PALLET_FIXED[pallet]['y'] + shift/100
    return
...
def save_csv(results, filepath=None, debug=False):
    ...
    if(filepath == None):
        ...
        with open(filepath, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=['timestamp', 'pallet', 'destination', 'success', 'pallet_y_shift', 'distance', 'duration_sec'])
            writer.writeheader()
...
def main():
    ...
    total = len(combos) * len(Y_SHIFT) * args.rounds
    print(f"=== Mission 3: Evaluation ===")
    print(f"Pallets: {pallets}")
    print(f"Destinations: {destinations}")
    print(f"Shift: {Y_SHIFT}")
